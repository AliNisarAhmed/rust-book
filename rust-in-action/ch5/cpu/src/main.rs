...
struct CPU {
    registers: [u8; 16],
    position_in_memory: usize, // commonly called program counter in CPU parlance
    memory: [u8; 0x1000],      // 0x1000 = 4096 => memory is 4096 Bytes or 4kB
    stack: [u16; 16],
    stack_pointer: usize,
}
...
impl CPU {
    ...
    // (6xkk) LS sets the value kk into register vx
    ...
    /// (00EE) RET return from the current function
    fn ret(&mut self) {
    ...
    fn add_xy(&mut self, x: u8, y: u8) {
        ...
        // CHIP-8 uses the last, 16th, registers as a carry flag
        if overflow {
